"""
Evaluation script for the ShortVid-Bench Benchmark.
Dataset: https://huggingface.co/datasets/TencentARC/ShortVid-Bench

Usage:
    python eval/eval_shortvideobench.py \
        --model-path Qwen/Qwen2.5-Omni-7B \
        --data-path /path/to/ShortVid-Bench-1k.json \
...
def parse_args():
    parser = argparse.ArgumentParser(description="ShortVid-Bench Evaluation")
    parser.add_argument('--model-path', type=str, default="Qwen/Qwen2.5-Omni-7B", help="Path or HuggingFace ID of the model")
    parser.add_argument('--data-path', type=str, required=True, help="Path to ShortVid-Bench-1k.json")
    parser.add_argument('--video-dir', type=str, required=True, help="Path to ShortVid-Bench video directory")
    ...
    parser.add_argument('--OMNIZIP_G', type=int, default=3, help='Number of tokens merged per anchor')
...
def evaluate_sample(sample, model, processor, video_dir, logger):
    ...
    try:
        ...
        logger.info(f"Processed {sample['video']}: GT={correct_answer} Pred={predicted_answer} {'correct' if is_correct else 'wrong'}")
        return {
            "video": sample["video"],
            "question": question,
        ...
        logger.error(f"Error processing {sample['video']}: {e}")
        return {
            "video": sample["video"],
            "question": question,
            "candidates": candidates,
...
def calculate_accuracy(results: List[Dict]) -> Dict:
    """Calculate overall and per-problem-type accuracy."""
    total = len(results)
    correct = sum(1 for r in results if r["is_correct"])

    type_stats = defaultdict(lambda: {"correct": 0, "total": 0})
    for r in results:
        type_stats[r["problem_type"]]["total"] += 1
        if r["is_correct"]:
            type_stats[r["problem_type"]]["correct"] += 1
...
def main():
    ...
    output_dir = os.path.join("logs", "results_shortvideobench", f"{current_time}_{method_tag}")
    ...
    logging.basicConfig(
        ...
        handlers=[
            logging.FileHandler(os.path.join(output_dir, 'eval_shortvideobench.log')),
    ...
    logger.info("Accuracy by Problem Type:")
    for ptype, stats in accuracy_metrics['problem_type_accuracy'].items():
        logger.info(f"  {ptype}: {stats['accuracy']:.4f} ({stats['correct']}/{stats['total']})")
