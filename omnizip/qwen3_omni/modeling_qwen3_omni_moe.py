...
class Qwen3OmniMoeAudioAttention(nn.Module):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if return_logits:
            # Per-token audio salience: for each cu_seqlens chunk, do a local
            # QK softmax and sum attention mass per key across queries & heads.
            # This matches the chunked attention semantics used by the encoder
            # and avoids the O(T^2) full attention map.
            with torch.no_grad():
                q = query_states.squeeze(0)  # [H, T, D]
                k = key_states.squeeze(0)    # [H, T, D]
                salience = torch.zeros(q.shape[1], device=q.device, dtype=q.dtype)
                cs = cu_seqlens.tolist()
                for i in range(len(cs) - 1):
                    s, e = int(cs[i]), int(cs[i + 1])
                    if e <= s:
                        continue
                    qk = torch.matmul(q[:, s:e, :], k[:, s:e, :].transpose(-1, -2)) * self.scaling
                    prob = F.softmax(qk, dim=-1)
                    salience[s:e] = prob.mean(dim=0).sum(dim=0)
        return attn_output, salience
...
class Qwen3OmniMoeAudioEncoderLayer(GradientCheckpointingLayer):
    ...
    def forward(
        ...
        **kwargs,
        ...
        hidden_states, salience = self.self_attn(
...
class Qwen3OmniMoeAudioEncoder(Qwen3OmniMoePreTrainedModel):
    ...
    def forward(
        ...
        for idx, encoder_layer in enumerate(self.layers):
            ...
            layer_outputs, layer_salience = encoder_layer(
                hidden_states,
                cu_seqlens,
        ...
        return BaseModelOutput(last_hidden_state=hidden_states), salience
...
class Qwen3OmniMoeThinkerTextModel(Qwen3OmniMoePreTrainedModel):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        pruning_mask (`torch.Tensor` of shape `(num_visual_tokens,)`, *optional*):
            Boolean mask over the original (pre-pruning) visual tokens indicating which ones survived
            OmniZip compression. Used by DeepStack to select the subset of multiscale visual features
            that align with the pruned hidden-state positions.
        """
    ...
    def _deepstack_process(self, hidden_states, visual_pos_masks, visual_embeds, pruning_mask=None):
        ...
        if pruning_mask is None:
            ...
            local_this = hidden_states[visual_pos_masks, :].clone() + visual_embeds[pruning_mask, :]
...
class Qwen3OmniMoeThinkerForConditionalGeneration(
    ...
    def get_audio_features(
        ...
        audio_outputs, attn_logits = self.audio_tower(
            input_features,
    ...
    def forward(
        ...
        **kwargs,
        ...
        if input_features is not None:
            audio_features, attn_logits = self.get_audio_features(
                input_features,
        ...
        if (
            ...
            omnizip_config = getattr(self, "omnizip_config", None) or {
                "rho_audio": 0.4,
                "rho_video": 0.7,
                "g": 3,
            ...
            video_indices_mask = (input_ids == self.config.video_token_id)
            global_mask_expanded = global_mask.unsqueeze(0).expand(input_ids.shape[0], -1)
            vision_pruning_mask = global_mask_expanded[video_indices_mask]
        ...
        outputs = self.model(
            ...
            pruning_mask=vision_pruning_mask,
            **kwargs,
        )
