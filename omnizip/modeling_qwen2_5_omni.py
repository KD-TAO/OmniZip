...
class Qwen2_5OmniThinkerForConditionalGeneration(Qwen2_5OmniPreTrainedModelForConditionalGeneration, GenerationMixin):
    ...
    def build_video_prune_mask(
        ...
        k: float,
        normalize: bool = True,
        keyframe_audio_mode: str = "none",
    ) -> torch.Tensor:
        """
        仅修剪 video tokens，返回一维 keep_mask (N,)。思路：
        1) 将每个 video 段按帧拆分：能整除4则四帧，否则两帧；若相邻两个段都是2帧，则跨音频并成一个4帧组处理。
        2) 组内相邻帧按相同偏移对齐，计算余弦相似度(A1-A2, A2-A3, A3-A4)；
        以“与相邻帧平均相似度”作为帧的非关键性分数，组内分数最小者判为关键帧（并列可多于1）。
        3) 非关键帧：以就近关键帧方向(左或右)的相邻帧为参考，对齐逐位置相似度作为“冗余分数”。
        4) 关键帧：默认不修剪；若 keyframe_audio_mode != 'none'，则用组后继的音频段做跨模态打分：
        - 取该音频段的均值向量(跨 batch 与该段 token 平均)，对关键帧逐位置计算与该均值的余弦相似度。
        - keyframe_audio_mode='prune_high'：相似度越高越易被剪（认为音视频强相关位置可由音频/其他帧表达）
        - keyframe_audio_mode='prune_low' ：相似度越低越易被剪（认为与音频弱相关的位置价值低）
        5) 汇总所有 video 位置的分数，按分数从高到低删除 round(k * #video) 个位置；非 video 全保留。

        参数:
            k: 总体在 video 上的修剪率(0~1)
            normalize: 是否先做 L2 归一化后再算余弦相似度（默认 True）
            keyframe_audio_mode: 'none' | 'prune_high' | 'prune_low'
                - 建议先试 'prune_high' 与 'prune_low' 两种方向，比较效果

        返回:
            keep_mask: (N,) bool，True=保留，False=修剪
        """
        assert inputs_embeds.dim() == 3 and input_ids.dim() == 2
        ...
        if normalize:
            eps = 1e-6
            emb = inputs_embeds / (inputs_embeds.norm(dim=-1, keepdim=True) + eps)
        else:
            emb = inputs_embeds

        # 标记 video / audio 在列维度是否出现
        is_video = (input_ids == video_token_id).any(dim=0)  # (N,)
        is_audio = (input_ids == audio_token_id).any(dim=0)  # (N,)
        ...
        for i in range(N):
            if is_video[i]:
                ...
                if in_seg:
                    segments.append((st, i - 1))
                    in_seg = False
        if in_seg:
            segments.append((st, N - 1))

        # 按帧拆分：能被4整除->4帧；否则->2帧；极短段则1帧
        def split_frames(seg):
            s, e = seg
            L = e - s + 1
            if L >= 4 and L % 4 == 0:
                q = L // 4
                return [(s + 0*q, s + 1*q - 1),
                        (s + 1*q, s + 2*q - 1),
                        (s + 2*q, s + 3*q - 1),
                        (s + 3*q, e)]
            # fallback: 两帧
            if L >= 2:
                q = L // 2
                return [(s, s + q - 1), (s + q, e)]
            # 极短：一帧
            return [(s, e)]

        # 将相邻两个“仅2帧”的 video 段跨音频合并成4帧组
        groups = []  # 每组是若干帧的[(s,e), ...]，长度为4或2或1
        # 记录每个 video 段之后的 audio 段（用于跨模态关键帧打分）
        audio_after_segment = {}  # seg_index -> (a_s, a_e) or None

        # 先枚举所有 audio 段，方便查询“段后继音频”
        audio_segments = []
        in_a, sa = False, 0
        for i in range(N):
            if is_audio[i]:
                ...
                if in_a:
                    audio_segments.append((sa, i - 1)); in_a = False
        if in_a:
            audio_segments.append((sa, N - 1))

        # 给每个 video 段找它之后最近的音频段（起点在段尾之后）
        def find_next_audio(after_idx):
            for (as_, ae_) in audio_segments:
                if as_ > after_idx:
                    return (as_, ae_)
            return None

        for si, seg in enumerate(segments):
            audio_after_segment[si] = find_next_audio(seg[1])

        i = 0
        while i < len(segments):
            fr = split_frames(segments[i])
            if len(fr) == 2 and (i + 1) < len(segments):
                fr_next = split_frames(segments[i + 1])
                if len(fr_next) == 2:
                    # 合并成四帧组
                    groups.append({
                        "frames": fr + fr_next,
                        # 该组的“后继音频”取第二个段之后的音频（更靠后）
                        "audio": audio_after_segment.get(i + 1, None)
                    })
                    i += 2
                    continue
            groups.append({
                "frames": fr,
                "audio": audio_after_segment.get(i, None)
            })
            i += 1

        # 工具：两帧对齐逐位置相似度（返回对齐到各自起点的索引与相似度）
        def pairwise_sims(frame_a, frame_b):
            sa, ea = frame_a; sb, eb = frame_b
            La = ea - sa + 1; Lb = eb - sb + 1
            L = min(La, Lb)
            if L <= 0:
                return None, None, None
            idx_a = torch.arange(sa, sa + L, device=device)
            idx_b = torch.arange(sb, sb + L, device=device)
            v1 = emb[:, idx_a, :]   # (B, L, D)
            v2 = emb[:, idx_b, :]   # (B, L, D)
            # 余弦相似度：若未归一化，则这是内积；归一化=True时即cosine
            sim = (v1 * v2).sum(dim=-1).mean(dim=0)  # (L,)
            return idx_a, idx_b, sim

        # 计算每个位置的“被剪冗余分数”
        scores = torch.full((N,), float("-inf"), device=device)

        for g in groups:
            frs = g["frames"]
            m = len(frs)
            if m == 1:
                # 单帧组 -> 默认关键帧；若允许跨模态关键帧剪裁，则用音频打分
                key_indices = [0]
                non_key = []
            else:
                # 相邻帧相似度
                adj = []
                for j in range(m - 1):
                    adj.append(pairwise_sims(frs[j], frs[j + 1]))  # (ia, ib, sim)

                # 帧级非关键性分数：与相邻帧的平均相似度
                frame_scores = []
                for j in range(m):
                    vals = []
                    if j - 1 >= 0 and adj[j - 1][2] is not None:
                        vals.append(adj[j - 1][2].mean())
                    if j <= m - 2 and adj[j][2] is not None:
                        vals.append(adj[j][2].mean())
                    if len(vals) == 0:
                        frame_scores.append(torch.tensor(0.0, device=device))
                    else:
                        frame_scores.append(torch.stack(vals).mean())
                frame_scores = torch.stack(frame_scores)  # (m,)
                min_val = frame_scores.min()
                key_idx = (frame_scores <= (min_val + 1e-6)).nonzero(as_tuple=False).squeeze(-1).tolist()
                key_indices = key_idx
                non_key = [j for j in range(m) if j not in key_indices]

            # 非关键帧：用就近关键方向的一侧相邻对齐相似度打分（越高越冗余）
            for j in non_key:
                # 选择就近关键帧的方向：左或右（按距离）
                left_keys = [k for k in key_indices if k < j]
                right_keys = [k for k in key_indices if k > j]
                left_dist = (j - max(left_keys)) if left_keys else 10**9
                right_dist = (min(right_keys) - j) if right_keys else 10**9

                if left_dist <= right_dist and left_dist != 10**9:
                    # 用左邻相似度（对齐到当前帧）
                    ia, ib, sim = pairwise_sims(frs[j - 1], frs[j])
                    if ib is not None:
                        scores[ib] = torch.maximum(scores[ib], sim)
                elif right_dist != 10**9:
                    # 用右邻相似度（对齐到当前帧）
                    ia, ib, sim = pairwise_sims(frs[j], frs[j + 1])
                    if ia is not None:
                        scores[ia] = torch.maximum(scores[ia], sim)
                else:
                    # 没有关键帧（理论上不会发生，因为至少有1个关键帧），跳过
                    continue

            # 关键帧：若启用跨模态关键帧修剪，则用“关键帧 vs 该组后继音频”的相似度打分
            if keyframe_audio_mode != "none" and g["audio"] is not None:
                a_s, a_e = g["audio"]
                if a_e >= a_s:
                    # 音频均值向量（跨 batch 与时间）
                    audio_vec = emb[:, a_s:(a_e + 1), :].mean(dim=(0, 1), keepdim=False)  # (D,)
                    # 若未归一化，此处做一次安全归一
                    if not normalize:
                        denom = audio_vec.norm() + 1e-6
                        audio_vec = audio_vec / denom

                    for j in key_indices:
                        s, e = frs[j]
                        idx = torch.arange(s, e + 1, device=device)
                        v = emb[:, idx, :].mean(dim=0)  # (L,D) -> 这里用逐位置还是均值？
                        # 为逐位置打分：对每个位置，使用该位置的向量与 audio_vec 的点积/余弦
                        v_pos = emb[:, idx, :].mean(dim=0)  # (L,D)（跨 batch 均值）
                        if not normalize:
                            # 转换为余弦
                            v_pos = v_pos / (v_pos.norm(dim=-1, keepdim=True) + 1e-6)
                        # 相似度 (L,)
                        sim_k = (v_pos * audio_vec.unsqueeze(0)).sum(dim=-1)

                        if keyframe_audio_mode == "prune_high":
                            # 相似度越高越冗余 -> 越易被剪
                            scores[idx] = torch.maximum(scores[idx], sim_k)
                        elif keyframe_audio_mode == "prune_low":
                            # 相似度越低越冗余 -> 用负相似度作为分数
                            scores[idx] = torch.maximum(scores[idx], -sim_k)
                        # 其他值忽略

        # 仅在 video 上参与排序
        vid_idx = torch.nonzero(is_video, as_tuple=False).squeeze(-1)
        num_video = int(vid_idx.numel())
        if num_video == 0:
            return torch.ones(N, device=device, dtype=torch.bool)

        prune_num = int(round(k * num_video))
        prune_num = max(0, min(prune_num, num_video))
        if prune_num == 0:
            return torch.ones(N, device=device, dtype=torch.bool)

        vid_scores = scores[vid_idx]
        # 将无效(-inf)分数置为非常小的值，避免被选中
        valid = torch.isfinite(vid_scores)
        if valid.any():
            # 用一个非常小的占位，确保不会被 topk 选上
            very_low = torch.finfo(vid_scores.dtype).min / 2
            safe_scores = torch.where(valid, vid_scores, torch.tensor(very_low, device=device, dtype=vid_scores.dtype))
        else:
            # 若没有任何有效分数（极少见），直接不修剪
            return torch.ones(N, device=device, dtype=torch.bool)

        # 选出要剪的位置
        _, topk_pos = torch.topk(safe_scores, k=prune_num, largest=True, sorted=False)
        to_prune = vid_idx[topk_pos]
